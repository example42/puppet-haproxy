...
# = Define: haproxy::conf
#
# With this define you can manage any haproxy configuration file
#
# == Parameters
#
# [*template*]
...
#   Example: template => 'site/haproxy/my.conf.erb',
#
# [*content*]
...
#   Example: source => 'puppet:///site/haproxy/my.conf',
#
# [*ensure*]
...
define haproxy::conf (
  ...
  $manage_path    = pickx($path, "${haproxy::config_dir_path}/${name}")
  $manage_content = default_content($content, $template)
  $manage_mode    = pickx($mode, $haproxy::config_file_mode)
  $manage_owner   = pickx($owner, $haproxy::config_file_owner)
  $manage_group   = pickx($group, $haproxy::config_file_group)
  $manage_require = pickx($config_file_require, $haproxy::config_file_require)
  $manage_notify  = $config_file_notify ? {
    'class_default' => $haproxy::manage_config_file_notify,
    default         => $config_file_notify,
  }

  file { "haproxy_conf_${name}":
    ensure  => $ensure,
    source  => $source,
